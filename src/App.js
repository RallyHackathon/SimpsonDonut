Ext.define('CustomApp', {
  ...
  transformDataToTree: function(snapshots) {
    ...
    _.each(_.pluck(sortedData, 'data'), function (data) {
      ...
      if (hl === 2) {
        tree[0]["children"][current] = {"name": name, "data": String(current), "children": {}};
      }
      else {
        parent = tree[0];
        ...
        parent["children"][current] = {"name": name, "data": String(current), "children": {}};
      }
    });
    
    this.treeToJSON(tree, treeSize);
  },
  ...
  objectToArray: function(obj) {
    ...
    for (key in obj["children"]) {
      ...
      ret.push(this.objectToArray(obj["children"][key]));
  ...
  treeToJSON: function(tree, treeSize) {
    // var json = [];
    // var nVisited = 0;
    // var current = tree[0];

    // while (nVisited < treeSize) {
    //   nVisited += Object.keys(current.children).length


    // }
    json = this.objectToArray(tree[0])["children"];
